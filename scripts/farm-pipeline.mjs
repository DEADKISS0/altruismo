...
async function callLLM(prompt) {
  ...
  if (LLM.provider === 'anthropic') {
    res = await fetch(LLM.url, {
      ...
      body: JSON.stringify({ model: LLM.model, system, messages: [userMessage], temperature: 0.7, max_tokens: 1500 }),
    });
  } else {
    res = await fetch(LLM.url, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${LLM.key}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: LLM.model, messages: [{ role: 'system', content: system }, userMessage], temperature: 0.7, max_tokens: 1500 }),
    });
  }
